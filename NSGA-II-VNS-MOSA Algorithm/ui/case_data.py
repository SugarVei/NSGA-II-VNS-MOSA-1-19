...
DEFAULT_ALGORITHM_PARAMS = {
    ...
    'NSGA-II': {
        'pop_size': 200,              # 种群大小
        'n_generations': 100,         # 进化代数（对比算法减少迭代以加速）
        'crossover_prob': 0.95,       # 田口最优: Pc=0.95
        'mutation_prob': 0.15,        # 田口最优: Pm=0.15
    },
    # MOSA: T0=100, max iterations=200, α=0.95
    'MOSA': {
        'initial_temp': 100.0,        # 初始温度 田口最优: T0=100
        'cooling_rate': 0.95,         # 降温速率 田口最优: α=0.95
        'final_temp': 0.001,          # 终止温度
        'max_iterations': 100,        # 最大迭代次数（对比算法减少迭代以加速）
    },
    # MOEA/D: N=200, Pc=0.95, Pm=0.15, T=40, weight vectors=N
    'MOEA/D': {
        'pop_size': 200,              # 种群大小
        'n_generations': 100,         # 进化代数（对比算法减少迭代以加速）
        'neighborhood_size': 40,      # 邻域大小 田口最优: T=40
        'delta': 0.9,                 # 邻域选择概率
        'nr': 2,                      # 最大替换数
        'crossover_prob': 0.95,       # 田口最优: Pc=0.95
        'mutation_prob': 0.15,        # 田口最优: Pm=0.15
    ...
    'SPEA2': {
        'pop_size': 200,              # 种群大小
        'n_generations': 100,         # 进化代数（对比算法减少迭代以加速）
        'archive_size': 100,          # 档案大小 田口最优: archive size=100
        'crossover_prob': 0.95,       # 田口最优: Pc=0.95
        'mutation_prob': 0.15,        # 田口最优: Pm=0.15
    },
    # MOPSO: swarm size N=200, max iterations=200, w=0.5, c1=c2=1.5
    'MOPSO': {
        'swarm_size': 200,            # 粒子群大小
        'max_iterations': 100,        # 最大迭代次数（对比算法减少迭代以加速）
        'w': 0.5,                     # 惯性权重 田口最优: w=0.5
        'c1': 1.5,                    # 认知系数 田口最优: c1=1.5
        'c2': 1.5,                    # 社会系数 田口最优: c2=1.5
        'repository_size': 200,       # 仓库大小
        'mutation_prob': 0.1,         # 变异概率
    },
    # NSGA-II-VNS: N=200, Pc=0.95, Pm=0.15, VNS iterations=10
    'NSGA2-VNS': {
        'pop_size': 200,              # 种群大小
        'n_generations': 100,         # 进化代数（对比算法减少迭代以加速）
        'crossover_prob': 0.95,       # 田口最优: Pc=0.95
        'mutation_prob': 0.15,        # 田口最优: Pm=0.15
        'vns_iterations': 10,         # VNS迭代次数
    },
    # NSGA-II-MOSA: N=200, Pc=0.95, Pm=0.15, T0=1000, α=0.95
    'NSGA2-MOSA': {
        'pop_size': 200,              # 种群大小
        'n_generations': 100,         # 进化代数（对比算法减少迭代以加速）
        'crossover_prob': 0.95,       # 田口最优: Pc=0.95
        'mutation_prob': 0.15,        # 田口最优: Pm=0.15
        'initial_temp': 1000.0,       # 初始温度 田口最优: T0=1000
        'cooling_rate': 0.95,         # 降温速率 田口最优: α=0.95
    },
    # NSGA-II-VNS-MOSA: 论文核心算法
    'NSGA2-VNS-MOSA': {
        'pop_size': 200,              # 种群大小
        'n_generations': 100,         # 进化代数（优化后降低以加速验证）
        'crossover_prob': 0.95,       # 田口最优: Pc=0.95
        'mutation_prob': 0.15,        # 田口最优: Pm=0.15
        'initial_temp': 1000.0,       # 初始温度 田口最优: T0=1000
        'cooling_rate': 0.95,         # 降温速率 田口最优: α=0.95
        'vns_max_iters': 5,           # VNS最大迭代次数（优化后降低以加速）
        'elite_ratio': 0.1,           # 精英比例（优化后降低以加速）
    },
}
